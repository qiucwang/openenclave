...
static int _parse_args(int argc, const char* argv[])
{
    ...
    while (i < argc)
    {
        if (strcmp(INPUT_PARAM_EVIDENCE_FORMAT, argv[i]) == 0)
        {
            ...
            if (strcasecmp(format, INPUT_PARAM_EVIDENCE_FORMAT_ECDSA) == 0)
                _params.evidence_format = &_ecdsa_quote_uuid;
            else if (
                strcasecmp(format, INPUT_PARAM_EVIDENCE_FORMAT_REPORT) == 0)
                _params.evidence_format = &_legacy_report_remote;
            else if (strcasecmp(format, INPUT_PARAM_EVIDENCE_FORMAT_QUOTE) == 0)
                _params.evidence_format = &raw_sgx_ecdsa_quote_uuid;
            else if (strcasecmp(format, INPUT_PARAM_EVIDENCE_FORMAT_CERT) == 0)
...
static oe_result_t _verify_evidence(oe_enclave_t* enclave)
{
    ...
    {
        result = verify_oe_certificate(enclave, _params.evidence_filename);
    }
    else
    {
...
int main(int argc, const char* argv[])
{
    ...
    const char* enclave_filename = nullptr;
    ...
    enclave_filename = _params.enclave_filename;
