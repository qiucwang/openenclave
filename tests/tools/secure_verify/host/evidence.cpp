    ...
    const char* certificate_filename)
{
    ...
    if (!read_binary_file(
            certificate_filename, &certificate_data, &certificate_size))
    {
        result = OE_INVALID_PARAMETER;
        goto done;
    }

    verify_plugin_certificate(
        enclave, &result, certificate_data, certificate_size);

    OE_CHECK_MSG(
        result,
        "Failed to verify_plugin_certificate. Error: %s\n",
        oe_result_str(result));
